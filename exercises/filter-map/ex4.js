...
*/
  ...
  function getActivitiesMembers(activities, persons) {
    let results = activities.map(function(activitie) {
      let toto = {};
      toto.activity = activitie;
      toto.persons = [];
      let tab = persons.map(function(person) {
        
        for (let i=0;i<person.activities.length;i++) {

          if (person.activities[i] === activitie) {
            
            toto.persons.push(person.name);
          };
        };    
      });
