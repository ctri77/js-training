...
function getCirclesProperties(radiuses) {
  let tab = radiuses.map(function (radiuse) {
    let result = {};
    result.radius = radiuse;
    result.circumference = Number.parseFloat(2 * Math.PI * radiuse).toFixed(3);
    result.surface = Number.parseFloat(Math.PI * radiuse * radiuse).toFixed(3);
    return result;
  });
